...
def fit(dimensions, datapoints):
    ...
    weights = np.matrix([1  for i in range(0, dimensions-1)]).T
    y_target = np.dot(x_random, weights)
    y = np.array(np.sin(x_orr))
    weights_n = (np.matrix([1  for i in range(0, dimensions-1)]) * 0.7).T
    initial_learning_rates = np.matrix([[0.001],[0.00001],[0.000001]])
    error_mat=[]
    def p1(x, weights_n):
